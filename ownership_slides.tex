...
% =====================================================================
% ENCODING & FONTS  (pdfLaTeX-compatible)
% =====================================================================
\usepackage[T1]{fontenc}          % Modern output font encoding
...
\begin{frame}{HJB Equations $\Longrightarrow$ Riccati Equations}
  ...
  \begin{itemize}
    ...
    \item $\boldsymbol{X}^{i}$ is the solution of stacked algebraic Riccati equations \hyperlink{riccati}{\beamerbutton{Riccati}}
...
\begin{frame}{Intuition of Why the Model Has the BGP}
  ...
  \end{itemize}
    \begin{center}
      ...
      \par\end{center}
  \begin{itemize}
    \item Linear and quadratic term in $\color{uclaBlue}\boldsymbol{q}_{t}$ of output grow at the same rate: \hyperlink{summary}{\beamerbutton{Equilibrium Summary}}
          \[
...
\begin{frame}{Expected Growth Rate and Utility}
  ...
  \begin{itemize}
    ...
    \[
    \begin{array}{cl}
    \log Y_{t} & =\log\left(\boldsymbol{z}_{t}^{T}\boldsymbol{Q}\boldsymbol{z}_{t}\right)\\
    d\boldsymbol{z}_{t} & =\boldsymbol{\Phi}\boldsymbol{z}_{t}dt+\gamma\boldsymbol{z}_{t} \circ d\boldsymbol{W}_{t}
    \end{array}
    \Longrightarrow \quad \boldsymbol{E}_{t}\left[\left.g_{t}\right|\boldsymbol{z}_{t}\right]
    \]
    \item Expected utility is expressed in quadratic form: \hfill \hyperlink{X}{\beamerbutton{X}}
          \[
            \boldsymbol{E}_{t}\left[\left.\int_{t}^{\infty}\exp\left(-\rho s\right)C_{s}ds\right|\boldsymbol{z}_{t}\right]=\boldsymbol{z}_{t}^{T}X\boldsymbol{z}_{t}
          \]
    \item Solve the equilibrium once $\Longrightarrow$ Can compute expected
          growth and utility for any $\boldsymbol{z}_{t}$
  \end{itemize}
\end{frame}
...
\end{frame}
%
\begin{frame}{Technology Spillover $\boldsymbol{\Omega}=\beta\times$Technological Proximity $\boldsymbol{\widetilde{\Omega}}$ \hyperlink{first_stage}{\beamerbutton{First Stage}}}
...
\begin{frame}{Total Output}
  ...
  \begin{tabular}{@{} c *{5}{c} @{}} 
    ...
    \midrule
    \shortstack[t]{Only Business Steal \\ $\boldsymbol{\Omega}=[0]$} 
      & \visible<2->{91.30} 
      & \visible<2->{91.02} 
      & \visible<2->{90.78} 
      & \visible<2->{89.08} 
      & \visible<2->{89.17} \\
    \midrule
    \shortstack[t]{Only Tech Spill \\ $\boldsymbol{\Sigma}=\boldsymbol{I},\,\zeta/L=0$} 
      & \visible<3->{75.00} 
      & \visible<3->{75.00} 
      & \visible<3->{75.00} 
      & \visible<3->{75.00} 
...
\begin{frame}{Riccati Equations}
  \begin{itemize}
    \item  \label{riccati} $V^{i}\left(\boldsymbol{z}\right)=\boldsymbol{z}^{T}\boldsymbol{X}^{i}\boldsymbol{z}$ where
          $\boldsymbol{X}^{i}$ is the solution of the stacked Riccati equation
          {\small
          \[
            0=\boldsymbol{Q}^{i}-\mu^{2}\sum_{j}\kappa_{ij}\boldsymbol{X}_{j}^{j}\left(\boldsymbol{X}_{j}^{j}\right)^{T}+\left(\boldsymbol{\Phi}-\frac{1}{2}\left(\rho-\gamma^{2}\right)\boldsymbol{I}\right)^{T}\boldsymbol{X}^{i}+\boldsymbol{X}^{i}\left(\boldsymbol{\Phi}-\frac{1}{2}\left(\rho-\gamma^{2}\right)\boldsymbol{I}\right)
          \]
          }
          \begin{itemize}
            \item $\boldsymbol{X}_{i}^{i}\equiv$ the $i$ th column of $\boldsymbol{X}^{i}$
            \item $\boldsymbol{\Phi}\equiv\boldsymbol{\Omega}+\mu^{2}\left[\begin{array}{ccc}
              \boldsymbol{X}_{1}^{1} & \cdots & \boldsymbol{X}_{n}^{n}\end{array}\right]^{T}$
          \end{itemize} \medskip{}
    \item Algorithm: Given $\left[\begin{array}{ccc}
                \boldsymbol{X}_{\tau}^{1} & \cdots & \boldsymbol{X}_{\tau}^{n}\end{array}\right]$, update $\left[\begin{array}{ccc}
                \boldsymbol{X}_{\tau-\Delta}^{1} & \cdots & \boldsymbol{X}_{\tau-\Delta}^{n}\end{array}\right]$ by
          {\small
          \[
            -\frac{\boldsymbol{X}_{\tau}^{i}-\boldsymbol{X}_{\tau-\Delta}^{i}}{\Delta}=\boldsymbol{Q}^{i}-\mu^{2}\sum_{j}\kappa_{ij}\boldsymbol{X}_{j,\tau}^{j}\left(\boldsymbol{X}_{j,\tau}^{j}\right)^{T}+\left(\boldsymbol{\Phi}_{\tau}-\frac{1}{2}\left(\rho-\gamma^{2}\right)\boldsymbol{I}\right)^{T}\boldsymbol{X}_{\tau}^{i}+\boldsymbol{X}_{\tau}^{i}\left(\boldsymbol{\Phi}_{\tau}-\frac{1}{2}\left(\rho-\gamma^{2}\right)I\right)
          \]}
  \end{itemize}
...
\begin{frame}{Summary of Equilibrium}
  \begin{center}
    ...
    \begin{tabular}{ll}
      ...
      \hline
      Production strategy             & $\boldsymbol{q}_{t}=N\boldsymbol{z}_{t}$\tabularnewline
      R\&D strategy                   & $\boldsymbol{x}_{t}=\mu\tilde{X}\boldsymbol{z}_{t}$\tabularnewline
      Law of motion                   & $d\boldsymbol{z}_{t}=\left(\Omega\boldsymbol{z}_{t}+\mu\boldsymbol{x}_{t}\right)dt+\gamma\boldsymbol{z}_{t}dW_{t}$\tabularnewline
      Profit of final producers       & $\Pi_{t}^{F}/P_{t}=\boldsymbol{q}_{t}^{T}\left(\frac{1}{2}\Sigma\right)\boldsymbol{q}_{t}$\tabularnewline
      Total operating profit of firms & $\Pi_{t}/P_{t}=\boldsymbol{q}_{t}^{T}\left(\frac{1}{2}\Sigma\circ\left(K+K^{T}\right)\right)\boldsymbol{q}_{t}$\tabularnewline
      Labor income                    & $w_{t}L/P_{t}=\boldsymbol{q}_{t}^{T}\left(\frac{\zeta}{L}J\right)\boldsymbol{q}_{t}$\tabularnewline
      Output                          & $Y_{t}=\boldsymbol{q}_{t}^{T}\left(\frac{\zeta}{L}J+\frac{1}{2}\Sigma+\frac{1}{2}\Sigma\circ\left(K+K^{T}\right)\right)\boldsymbol{q}_{t}$\tabularnewline
      Consumption                     & $C_{t}=Y_{t}-\boldsymbol{x}_{t}^{T}\boldsymbol{x}_{t}$\tabularnewline
    \end{tabular}
    \par\end{center}
...
\begin{frame}{Output and Expected Utility }
  ...
  \label{X}
  \begin{itemize}
    \item Output: $Y_{t}=\boldsymbol{q}_{t}^{T}Q\boldsymbol{q}_{t}$ where
          \[
