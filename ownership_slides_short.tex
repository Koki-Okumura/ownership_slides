...
\begin{frame}{Ownership Structure $\Longrightarrow$ Economic Growth?}
  ...
  \begin{itemize}
    \item Corporate ownership is highly concentrated \citep{Backus2021-yt} \hfill \hyperlink{share}{\beamerbutton{Share}}
          ...
          \end{itemize}
          \medskip{} \pause
    \item Such concentration has led to a large literature and policy debates on common ownership
          \begin{itemize}
            \item Implications for innovation: \citet{Lopez2019-sl, Kini2024-kd, Anton2024-pw}
            \item Merger Guidelines raise concerns about the impact of common ownership on R\&D \hyperlink{guidelines}{\beamerbutton{Merger Guidelines}}
          \end{itemize}
          \medskip{}\pause
    \item Common ownership facilitates the internalization of inter-firm externalities
          \begin{itemize}
            \item \citet{Rotemberg1984-jz, Azar2021-uh, Ederer2024-rw}
          \end{itemize}
          \medskip{} \pause
    \item Two inter-firm externalities of innovation \citep{Aghion1992-zz,Bloom2013-pn}:
          \begin{itemize}
            \item[$-$] \alert{Business-stealing effect}
            \item[$+$] \alert{Technology spillover effect}
          \end{itemize}
          \medskip{} \pause
    \item \alert{The aggregate effects of common ownership on R\&D allocation and growth remain unknown}
  \end{itemize}
\end{frame}

\begin{frame}{Quantitative Endogenous Growth Model with Ownership Structure}
  \begin{itemize}
    \item Common ownership and technology spillovers span across industries $\Longrightarrow$ \\
          Firms internalize intra- and inter-industry externalities
          \begin{itemize}
            \item Innovation networks: \citet{Scherer1982-zz,Acemoglu2016-hk,Liu2024-bt}
          \end{itemize}
          \medskip{}\pause
    \item In canonical endogenous growth models, each industry is infinitesimally small $\Longrightarrow$ \\
          if any, strategic interactions occur only within an industry
          \begin{itemize}
            \item Monopolistic competition: \citet{Romer1990-cw,Klette2004-gz}
            \item Oligopoly with 2--4 firms in each industry: \citet{Aghion2001-yc,Cavenaile2023-lo}
          \end{itemize}
          \medskip{}\pause
    \item My framework is based on the endogenous growth model by \citet{Hopenhayn2024-ya}
          \begin{itemize}
            \item \alert{Hundreds or thousands of oligopolists} engage in a dynamic R\&D game
            \item \alert{Product market rivalry networks} \citep{Pellegrino2024-dn} and \alert{innovation networks}
            \item LQ differential game avoids the curse of dimensionality
          \end{itemize}
          \medskip{}\pause
    \item This paper incorporates \alert{ownership networks} into \citet{Hopenhayn2024-ya} $\Longrightarrow$
          \\ Overlap of networks determines the internalization of the two externalities
  \end{itemize}
\end{frame}

\begin{frame}{Model Environment}
  \begin{itemize}
    \item Continuous time \medskip{}\pause
    \item Many granular firms \medskip{}\pause
    \item Markov perfect equilibrium \medskip{}\pause
    \item Each firm produces a single differentiated product and engages in Cournot competition \medskip{}\pause
    \item Each firm invests in R\&D to accumulate knowledge capital $z_{i,t}$ \medskip{}\pause
    \item Strategic interaction through three (exogenous) inter-firm networks
          \medskip
          \begin{center}
            \begin{tabular}{l c l}
              \toprule
              Network        & Notation                      & Measurement                                           \\
              \midrule
              Ownership      & $\bm{K} = [\kappa_{ij}]$      & Weight firm $i$ places on firm $j$'s value            \\
              \addlinespace
              Product market & $\bm{\Sigma} = [\sigma_{ij}]$ & Substitutability between firms $i$ and $j$'s products \\
              \addlinespace
              Innovation     & $\bm{\Omega} = [\omega_{ij}]$ & Technology spillover through $z_{i,t}$ and $z_{j,t}$  \\
              \bottomrule
            \end{tabular}
          \end{center}
          \medskip
...
\begin{frame}{Key Properties of the Model}
  \begin{itemize}
    \item (Stochastic) Balanced growth path with endogenous growth rate
          \medskip{}
    \item<2-> More overlap in $\bm{K}$ and $\bm{\Sigma}$ $\Longrightarrow$ internalize business-stealing effects $\Longrightarrow$ R\&D $\downarrow$
          \medskip{}
    \item<3-> More overlap in $\bm{K}$ and $\bm{\Omega}$ $\Longrightarrow$ internalize technology spillovers $\Longrightarrow$ R\&D $\uparrow$
          \medskip{}
  \end{itemize}
  \begin{center}
    \begin{tabular}{l c l}
...
\begin{frame}{Identification of Networks}
  \begin{itemize}
  ...
  \begin{center}
    \begin{tabular}{l c p{7.8cm}}
      ...
      Network        & Notation                      & Measurement                                 \\
      \midrule
      Ownership      & $\bm{K} = [\kappa_{ij}]$      & Institutional investor shareholdings        \\
      \addlinespace
      Product market & $\bm{\Sigma} = [\sigma_{ij}]$ & Business descriptions \citep{Hoberg2016-jm} \\
      \addlinespace
      Innovation     & $\bm{\Omega} = [\omega_{ij}]$ & Patent classifications \citep{Jaffe1986-yz} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{frame}

\begin{frame}{Impact of Common Ownership on R\&D and Production}
  \begin{center}
    \setlength{\tabcolsep}{3pt}
    \renewcommand{\arraystretch}{1.15}
    \begin{tabular}{@{}lccc@{}}
      \toprule
      \multicolumn{1}{l}{Production ownership structure} & \alt<1,3>{\alert{Dispersed}}{Dispersed} & \alt<1,2>{\alert{Dispersed}}{Dispersed} & \alt<2,3>{\alert{Common}}{Common} \\
      \multicolumn{1}{l}{R\&D ownership structure}       & \alt<1,3>{\alert{Dispersed}}{Dispersed} & \alt<1,2>{\alert{Common}}{Common}       & \alt<2,3>{\alert{Common}}{Common} \\
      \midrule
      \shortstack[l]{Total Output (Dispersed: 100)}
                                                         & 100                                     & 100                                     & 97.3                              \\
      \alert{\shortstack[l]{Total R\&D (Dispersed: 100)}}
                                                         & \alt<1,3>{\alert{100}}{100}
                                                         & \alt<1,2>{\alert{69.8}}{69.8}
                                                         & \alt<2,3>{\alert{86.4}}{86.4}                                                                                         \\
  ...
  \medskip{}
  \begin{itemize}
    \item<1-> \alert<1>{Internalization of business-stealing $>$ technology spillovers}\medskip{}
    \item<2-> \alert<2>{Less product market competition $\Longrightarrow$ Private return on R\&D $\uparrow$}\medskip{}
    \item<3-> \alert<3>{Internalization of business-stealing $>$ technology spillovers $+$ less product market competition}
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusion}
  \begin{itemize}
    ...
    \item Potential applications:
          \begin{itemize}
            \item M\&A / FDI
            \item Conglomerate / Cross-shareholdings (e.g. Japan, Korea, Germany, AI companies \hyperlink{ai}{\beamerbutton{AI}})
          \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Related Literature}
  \begin{itemize}
    \item Competition and Innovation: \\
          {\footnotesize\citet{d-Aspremont1988-je,Kamien1992-la,Ericson1995-dq,Aghion2001-yc,Aghion2005-vw,Acemoglu2012-bj,Aghion2013-nq,Bloom2013-pn,Lopez2019-sl,Peters2020-sd,Akcigit2021-ns,Akcigit2023-zl,Cavenaile2023-lo}, \textbf{\citet{Hopenhayn2024-ya}}}\\
          \alert{Endogenous growth model with ownership networks}
          \medskip{}
    \item Common Ownership: \\
          {\footnotesize\citet{Rubinstein1983-pi,Rotemberg1984-jz,Gutierrez2017-wl,He2017-ix,Azar2018-cc,Azar2022-cn,Azar2021-uh,Anton2023-ej}, \textbf{\citet{Kini2024-kd,Anton2024-pw}}} \\
          \alert{Macroeconomic implications of common ownership for R\&D allocation and growth}
          \medskip{}
    \item Oligopoly / Hedonic Demand: \\
          {\footnotesize\citet{Lancaster1966-sg,Rosen1974-ep,Berry1995-lx,Nevo2001-ja,Neary2003-sn,Atkeson2008-zc,Autor2020-mr}, \textbf{\citet{Pellegrino2024-dn,Ederer2024-rw}}}\\
          \alert{Dynamic general equilibrium model / R\&D / technology spillovers}
  \end{itemize}
\end{frame}
...
\begin{frame}{Preference and Production Technology (1/2)}
  \begin{itemize}
    ...
    \item Linear-quadratic aggregator \citep{Pellegrino2024-dn}:
          ...
          \]\vspace{-1.5em}
          \begin{itemize}
            \item $\bm{b}_{t}=\left[b_{1,t},...,b_{n,t}\right]^{T}$: product quality vector (endogenous)
            \item \alert{$\bm{\Sigma}=\left[\sigma_{ij}\right]$: product-market rivalry matrix (exogenous) $\quad (\sigma_{ii} = 1)$}
          \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Preference and Production Technology (2/2)}
  \begin{itemize}
    \item Final goods price is normalized to 1\medskip{}
    \item Linear inverse demand: \[p_{i,t}=b_{i,t}-\sum_{j\neq i}\sigma_{ij}q_{j,t} - q_{i,t}\]
    \item CRS intermediate-good production technology: \[q_{i,t}=a_{i,t}l_{i,t}\]
    \item Each firm $i$ has knowledge capital $z_{i,t}$
          \medskip{}
    \item Firms allocate knowledge capital to improve labor productivity and product quality:
          \[
...
\begin{frame}{Law of Motion of Knowledge Capital}
  ...
  \]
  \begin{itemize}
    \item \alert{$\bm{\Omega}=\left[\omega_{ij}\right]$: technology spillover matrix / innovation networks}\medskip{}
...
\begin{frame}{Market Clearing}
  \begin{itemize}
    \item Final good market clearing:
          \[
            Y_{t} = C_{t}+\underbrace{\sum_{i}d_{i,t}}_{\text{R\&D input}}
          \]
...
\begin{frame}{Ownership Networks: Common Ownership Weights}
  ...
  \begin{itemize}
    \item \alert{$\bm{K}=\left[\kappa_{ij}\right]$: common ownership weights that firm $i$ places on the value of firm $j$ $\quad (\kappa_{ii} = 1)$}
          \medskip{}
    \item More overlapping ownership between firms $i$ and $j$ $\Longrightarrow$ higher $\kappa_{ij}$  \medskip{}
...
\begin{frame}{Proportional Influence \citep{Rotemberg1984-jz} }
  \label{rotemberg}
  \begin{itemize}
    \item Baseline corporate governance assumption to map shareholding data to $\alert{\kappa_{ij}}$
...
\begin{frame}{Markov Perfect Equilibrium}
  \begin{itemize}
    \item Given other firms' strategies, firm $i$ chooses $\left\{a_{i,t}, b_{i,t}, q_{i,t}, x_{i,t}\right\}_{t\geq0}$ to maximize
          \[
            \max_{\left\{a_{i,t}, b_{i,t}, q_{i,t}, x_{i,t}\right\}_{t\geq0}}\quad V_{i}\left(\bm{z}_{0}\right)\equiv\int_{0}^{\infty}\exp\left(-\rho t\right) \sum_{j}\kappa_{ij}\left(\underbrace{\pi_{j,t}}_{\text{Gross Profit}}-\underbrace{d_{j,t}}_{\text{R\&D Cost}}\right) dt
          \]
    \item Markov perfect equilibrium can be solved by the following steps:
          ...
          \begin{enumerate}
            \item \alert{Static Game:} For each $t$, choose $\left\{a_{i,t}, b_{i,t}, q_{i,t}\right\}$ to maximize $\sum_{j} \kappa_{ij} \pi_{j,t}$.
                  \medskip{}
            \item \alert{Dynamic Game:} Given the static strategy profile, choose $\left\{x_{i,t}\right\}$ to maximize $V_{i}\left(\bm{z}_{0}\right)$
          \end{enumerate}
  \end{itemize}
\end{frame}
...
\begin{frame}{Dynamic R\&D Game $\Longrightarrow$ Linear-Quadratic Differential Game}
  ...
  \begin{itemize}
    \item Given other firms' R\&D $\left\{ x_{j,t}\right\}_{j\neq i,\,t\geq0}$, firm $i$ chooses R\&D $\left\{ x_{i,t}\right\}_{t\geq0}$ to maximize
          \[
            \max_{\left\{ x_{i,t}\right\}_{t\geq0}}\quad V_{i}\left(\bm{z}_{0}\right)\equiv\int_{0}^{\infty}\exp\left(-\rho t\right) \sum_{j}\kappa_{ij}\left(\pi_{j,t}-d_{j,t}\right) dt
          \]
          \begin{itemize}
    ...
    \item Firm $i$'s HJB equation:
          \[
            \rho V_{i}\left(\bm{z}\right)=\max_{x_{i}}\left\{ \bm{z}^{T}\bm{Q}^{i}\bm{z}-\sum_{j}\kappa_{ij}x_{j}^{2}+V_{\bm{z}}^{i}\left(\bm{z}\right)\left[\bm{\Omega}\bm{z}+\mu\bm{x} - \delta\bm{z} \right]\right\}
          \]
  \end{itemize}
\end{frame}
...
\begin{frame}{HJB Equations $\Longrightarrow$ Riccati Equations}
  ...
  \label{hjb}
  \begin{itemize}
    \item Guess and verify $V_{i}\left(\bm{z}\right)=\bm{z}^{T}\bm{X}^{i}\bm{z}$
          (for any $\bm{z}$) \medskip{}
    \item $\bm{X}^{i}$ is the solution of stacked algebraic Riccati equations

          \hyperlink{riccati}{\beamerbutton{Riccati Equations}}\medskip{}
    \item Public \& patenting firms in the U.S. in our dataset $> 700$
          ...
          \begin{center}
            \begin{tabular}{@{}lccc@{}}
              \toprule
              \shortstack{Schumpeterian growth model                    \\ with oligopolistic competition}& Computation time & \shortstack{\# of firms\\in each game} & Productivity space \\
              \midrule
              \citet{Cavenaile2023-lo} & $O(2^n)$ & 4      & 6 grid     \\
              HO (2025), This paper    & $O(n^4)$ & $>$700 & Continuous \\
              \bottomrule
            \end{tabular}
          \end{center}
...
\begin{frame}{CES on BGP despite Non-CES Demand}
  ...
  \begin{itemize}
    ...
    \item (i) (consumer surplus / producer surplus) and (ii) markup stay the same
...
\begin{frame}{Lifetime Utility}
  ...
  \begin{itemize}
    ...
    \item Solve the equilibrium once $\Longrightarrow$ we can compute lifetime utility for any initial $\bm{z}_{t}$
          \medskip{}
    \item This property holds for expected utility even if we introduce (idiosyncratic / aggregate) shocks
          \medskip{}
...
\begin{frame}{Common Ownership Weights $\bm{K}$}
  \begin{itemize}
    \item \citet{Backus2021-yt} construct a dataset on institutional investors' holdings based on Form 13F
...
\begin{frame}{Distribution of Knowledge Capital $\bm{z}_t$}
  \begin{table}[htbp]
    ...
    \begin{tabular}{cl}
      ...
      \midrule
      $\pi_{i,t}$ & Gross profit (before R\&D cost) $=$ Revenue $-$ Cost of goods sold                                                          \\[6pt]
      $\bm{q}_t$  & \(\pi_{i,t}=\displaystyle\sum_{j}\kappa_{ij}\,\sigma_{ij}\,q_{i,t}\,q_{j,t}\)                                               \\[6pt]
      $\zeta/L$   & Matches sample firms' cost share (average markup)                                                                           \\[6pt]
      $\bm{z}_t$  & \(\displaystyle \bm{z}_{t}=\left\{2\frac{\zeta}{L}\bm{1}_{n\times n}+\bm{\Sigma}+\bm{K}\circ\bm{\Sigma}\right\}\bm{q}_{t}\) \\
      \bottomrule
    \end{tabular}
  \end{table}
  \begin{itemize}
    \item Observed $\bm{z}_{t}$ is used to
          \begin{enumerate}
            \item estimate the law of motion of knowledge capital, and
            \item identify the initial distribution of knowledge capital
          \end{enumerate}
  \end{itemize}
\end{frame}
...
\begin{frame}{Identification: Summary}
  ...
  \begin{table}[h]
    ...
    \end{tabular}
  \end{table}
  \hyperlink{network_correlations}{\beamerbutton{Network Correlation}}
\end{frame}
...
\begin{frame}{Model vs. Data: Comparison with \cite{Anton2024-pw}}
  \begin{itemize}
  ...
  \begin{center}
    ...
    \begin{tabular}{@{} c *{4}{c} @{}}
      ...
      \multirow{2}{*}{\shortstack[t]{Total R\&D in 2017                                                               \\ (Optimal R\&D: 100)}}
           & \multicolumn{4}{c}{Ownership Structure}                                                                  \\
      \cmidrule(lr){2-5}
           & Dispersed
           & Mean$=$1999
           & Baseline
      ...
      Both & 40.5                                    & \textcolor{uclaBlue}{38.7} & \textcolor{uclaBlue}{28.3} & 21.4 \\
      \midrule
...
\begin{frame}{Total R\&D, Growth Rate, Social Welfare, Firm Value Share}
  ...
  \begin{tabular}{@{} c *{4}{c} @{}}
    \toprule
    \multirow{2}{*}{\shortstack[t]{}} & \multicolumn{4}{c}{Ownership Structure}                                                                  \\
    \cmidrule(lr){2-5}
                                      & Dispersed
                                      & Mean$=$1999
                                      & Baseline
    ...
    \midrule
    \shortstack[l]{Economic Growth Rate (\%)}
                                      & 1.32                                    & \textcolor{uclaBlue}{1.31} & \textcolor{uclaBlue}{1.20} & 1.11 \\
    \midrule
    \shortstack[l]{CE Welfare (Optimal R\&D: 100)}
                                      & 94.9                                    & \textcolor{uclaBlue}{94.9} & \textcolor{uclaBlue}{94.4} & 93.5 \\
    \midrule
    \shortstack[l]{Firm Value Share (\%)}
                                      & 26.6                                    & 26.7                       & 27.2                       & 27.8 \\
    \bottomrule
...
\begin{frame}{Which Firms' R\&D Should be Subsidized?}
  ...
  \begin{center}
    ...
    \begin{tabular}{lcc}
      ...
      \hline
      Initial knowledge capital $z$   & $\begin{array}{c}\text{0.12}^{***}\\(\text{0.00085})\end{array}$ & $\begin{array}{c}\text{-0.00021}\\(\text{0.00016})\end{array}$                             \\
      Product market centrality       & $\begin{array}{c}\text{-16}^{***}\\(\text{0.29})\end{array}$     & $\begin{array}{c}\textcolor{uclaBlue}{\text{-1.1}^{***}}\\(\text{0.053})\end{array}$ \\
      Technology spillover centrality & $\begin{array}{c}\text{5.5}^{***}\\(\text{0.42})\end{array}$     & $\begin{array}{c}\textcolor{uclaBlue}{\text{1.5}^{***}}\\(\text{0.079})\end{array}$ \\
      Ownership structure centrality  & $\begin{array}{c}\text{-7.2}^{***}\\(\text{0.24})\end{array}$    & $\begin{array}{c}\textcolor{uclaBlue}{\text{0.58}^{***}}\\(\text{0.045})\end{array}$ \\
      Intercept                       & $\begin{array}{c}\text{-22}^{***}\\(\text{0.26})\end{array}$     & $\begin{array}{c}\text{0.90}^{***}\\(\text{0.048})\end{array}$                             \\
      \hline
      Observations                    & \multicolumn{1}{c}{740}                                          & \multicolumn{1}{c}{740}                                                                    \\
      $R^2$                           & \multicolumn{1}{c}{0.98}                                         & \multicolumn{1}{c}{0.56}                                                                   \\
      \hline\hline
    \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}{Alternative Corporate Governance Models}
  ...
  \begin{enumerate}
    \item Dispersed onwership: $\kappa^D_{ij}=0 \quad(i\neq j)$
          \medskip{}
    ...
    \begin{center}
      \begin{tabular}{@{}lcccccc@{}}
        ...
        \shortstack[l]{Total R\&D Expenditure}
         & 100                                     & \textcolor{uclaBlue}{69.8} & \textcolor{uclaBlue}{69.0} & \textcolor{uclaBlue}{77.5} & \textcolor{uclaBlue}{90.3} & \textcolor{uclaBlue}{90.4} \\
        \shortstack[l]{Growth Rate (\%) }
         & 1.32                                    & 1.20                       & 1.19                       & 1.23                       & 1.29                       & 1.29                       \\
        \shortstack[l]{CE Welfare}
         & 100                                     & 99.4                       & 99.4                       & 99.6                       & 99.9                       & 99.9                       \\
        \shortstack[l]{Firm Value Share (\%) }
         & 26.6                                    & 27.2                       & 27.2                       & 27.1                       & 26.8                       & 26.8                       \\
        \bottomrule
      \end{tabular}
    \end{center}
  }
...
\begin{frame}{Merger Guidelines \hyperlink{intro}{\beamerbutton{Back}}}
  \label{guidelines}
  \begin{quote}
    ``For example, \alert{the partial owner may decide not to develop a new product feature} to win market share from the firm in which it has acquired an interest, \alert{because doing so will reduce the value of its investment in its rival. }''
  \end{quote}
  \medskip{}
...
\begin{frame}{Static Profits}
  ...
  \begin{itemize}
    ...
    \item $\bm{q}_{t}=\bm{N}\bm{z}_{t}$ where $\bm{N}\equiv\left\{ 2\frac{\zeta}{L}\bm{1}_{n\times n}+\bm{\Sigma}+\bm{K}\circ\bm{\Sigma}\right\} ^{-1}$
...
\begin{frame}{Intuition of Externality}
  \label{static_equilibrium}
  \begin{itemize}
    \item Assume firms choose static variables $\left\{a_{i,t}, b_{i,t}, q_{i,t}\right\}$ to maximize static profits
    \item Equilibrium quantity $q_i = \frac{1}{2} z_i - \frac{1}{2}\sum_{k \neq i} \sigma_{ik} q_k - \sqrt{\zeta w_t}$ and profit $\pi_i = q_i^2$
  \end{itemize}
  \begin{align*}
...
\begin{frame}{Riccati Equations}
  \label{riccati}
  \begin{itemize}
    \item $V_{i}\left(\bm{z}\right)=\bm{z}^{T}\bm{X}^{i}\bm{z}$ where $\bm{X}^{i}$ is the solution of the stacked Riccati equation
            {\small
              \[
...
\begin{frame}{Summary of Equilibrium}
  ...
  \begin{center}
    \begin{tabular}{ll}
      ...
      Labor income                    & $w_{t}L/P_{t}=\bm{q}_{t}^{T}\left(\frac{\zeta}{L}\bm{1}_{n\times n}\right)\bm{q}_{t}$\tabularnewline
      Output                          & $Y_{t}=\bm{q}_{t}^{T}\left(\frac{\zeta}{L}\bm{1}_{n\times n}+\frac{1}{2}\bm{\Sigma}+\frac{1}{2}\bm{\Sigma}\circ\left(\bm{K}+\bm{K}^{T}\right)\right)\bm{q}_{t}$\tabularnewline
...
\begin{frame}{Output and Expected Utility}
  ...
  \begin{itemize}
    \item Output: $Y_{t}=\bm{q}_{t}^{T}\bm{Q}\bm{q}_{t}$ where
          \[
            \bm{Q}=\frac{\zeta}{L}\bm{1}_{n\times n}+\frac{1}{2}\bm{\Sigma}+\frac{1}{2}\bm{\Sigma}\circ\left(\bm{K}+\bm{K}^{T}\right)
          \]
